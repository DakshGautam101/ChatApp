...
export const processSingleUpload = async (file, userId, uploadId) => {
    const attachment = toAttachment(file);
    if (uploadId) {
        await UploadSession.findOneAndUpdate({ uploadId }, {
            uploadId,
            uploader: userId,
...
export const registerSessionService = async (userId, bodyData) => {
    ...
    const session = await UploadSession.findOneAndUpdate({ uploadId }, {
        $setOnInsert: {
            uploadId,
            uploader: userId,
...
export const markInterruptedService = async (uploadId) => {
    if (!uploadId)
        return;
    await UploadSession.findOneAndUpdate({ uploadId, status: { $ne: "completed" } }, { $set: { status: "interrupted" } });
};
