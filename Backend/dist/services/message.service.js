...
export const sendMessageService = async ({ userId, conversationId, content, attachments = [] }) => {
    ...
    await Conversation.updateOne({ _id: conversationId }, { $set: { lastMessage: { text: previewText, sender: userId, at: new Date() } } });
    ...
    for (const conv of conversations) {
        ...
        if (conv.type === "group") {
            ...
            if (memberCount === 2) {
                ...
                Conversation.updateOne({ _id: conv._id }, { $set: { type: "private", isConvertedFromGroup: true, formerGroupName: formerName, name: null, avatarUrl: null } }).catch(() => { });
                conv.type = "private";
