...
import { createGroupService, sendGroupInvitationService, getPendingGroupInvitationsService, respondToGroupInvitationService, searchUsersForGroupService, updateGroupAvatarService, updateMemberRoleService, leaveGroupService, kickMemberService, } from "../services/group.service.js";
    ...
    try {
        ...
        if (io) {
            if (action === "accepted" && conversation) {
                ...
                const participant = convObj.participants?.find((p) => (p.user?._id || p.user)?.toString() === userId.toString());
        ...
        return sendSuccess(res, 200, { invitation, conversation }, `Group invitation ${action} successfully`);
    }
    catch (error) {
