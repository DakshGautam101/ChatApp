    ...
    io.on("connection", (socket) => {
        ...
        socket.on("conversation:join", async (conversationId) => {
            try {
                ...
                const participant = conversation.participants.find((p) => (p.user?._id || p.user).toString() === userId.toString());
                ...
                if (undelivered.length) {
                    await Message.updateMany({ _id: { $in: undelivered.map((m) => m._id) } }, { $set: { status: "delivered" } });
        ...
        socket.on("message:react", async ({ conversationId, messageId, reactionType }) => {
            try {
                ...
                if (!message)
                    return;
                const existing = message.reactions.find((reaction) => (reaction.user._id || reaction.user).toString() === userId);
                if (existing) {
                    message.reactions = message.reactions.filter((reaction) => (reaction.user._id || reaction.user).toString() !== userId);
                }
                else {
        ...
        socket.on("message:read", async ({ conversationId, lastMessageId }) => {
            try {
                ...
                if (!conversation)
                    return;
                await Conversation.updateOne({ _id: conversationId, "participants.user": userId }, { $set: { "participants.$.lastReadMessageId": lastMessageId } });
                ...
                if (minDate)
                    msgFilter.createdAt = { $gte: minDate };
