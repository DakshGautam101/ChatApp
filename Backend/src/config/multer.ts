...
function checkChatFileType(req : Request, file:any, cb:Function) {
    const ext = path.extname(file.originalname).toLowerCase();
    if (allowedChatMimeTypes.includes(file.mimetype) && allowedChatExtensions.includes(ext)) {
        cb(null, true);
    } else {
        const error : CustomError = new Error("Unsupported file type. Supported types: JPEG, PNG, GIF, PDF, DOCX, MP4");
        error.statusCode = 400;
        cb(error, false);
    }
}

function checkAvatarFileType(req : Request, file:any, cb:Function) {
    const ext = path.extname(file.originalname).toLowerCase();
    if (allowedAvatarMimeTypes.includes(file.mimetype) && allowedAvatarExtensions.includes(ext)) {
        cb(null, true);
    } else {
        const error : CustomError = new Error("Unsupported avatar format. Only JPEG, PNG, GIF, and WEBP images are allowed");
        error.statusCode = 400;
        cb(error, false);
    }
}
