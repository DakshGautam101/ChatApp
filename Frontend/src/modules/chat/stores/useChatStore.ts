...
interface ChatStoreInterface {
    ...
    _conversationsRequestToken: number;

    fetchConversations: () => Promise<void>;
    openConversation: (conversation: ConversationInterface | any) => Promise<void>;
    closeConversation: () => void;
    loadOlderMessages: () => Promise<void>;
    addOptimisticMessage: (optimisticMsg: MessageInterface) => void;
    updateOptimisticAttachment: (
        messageId: string | undefined,
        itemId: string,
        patch: Partial<AttachmentInterface> | Record<string, any>
    ) => void;
    removeOptimisticMessage: (tempId: string) => void;
    sendMessage: (payload: string | SendMessagePayload) => Promise<void>;
    uploadFile: (file: File) => Promise<any>;
    uploadMultipleFiles: (files: File[]) => Promise<any>;
    reactToMessage: (messageId: string, reaction: string) => void;
    receiveMessage: (message: MessageInterface) => void;
    updateMessageStatus: (payload: string | MessageStatusPayload | any) => void;
    updateMessageReaction: (message: MessageInterface) => void;
    ...
    createGroup: (data: {
        name: string;
        members: UserInterface[] | string[] | any[];
    }) => Promise<any>;
    sendGroupInvitation: (groupId: string, payload: {
        receiverId: string;
    }) => Promise<any>;
    fetchGroupInvitations: () => Promise<any>;
    respondToGroupInvitation: (invitationId: string, action: string) => Promise<any>;
    addOrUpdateConversation: (conversation: ConversationInterface | any) => void;
    removeConversation: (conversationId: string) => void;
    updateGroupAvatar: (groupId: string, avatarUrl: string | null) => Promise<any>;
    updateMemberRole: (groupId: string, targetUserId: string, newRole: string) => Promise<any>;
    leaveGroup: (groupId: string) => Promise<void>;
    kickMember: (groupId: string, targetUserId: string) => Promise<any>;
    ...
    handleUserStatusEvent: (data: { userId: string; status: string }) => void;
}

function sortConversations(convs: ConversationInterface[] | any[]) {
...
const useChatStore = create<ChatStoreInterface>((set, get) => ({
    ...
    fetchConversations: async () => {
        ...
        try {
            ...
            const fetched = rawFetched.filter((c: any) => {
                if (c.type === "group") {
                    const validMembers = (c.participants || []).filter((p: any) => p.user && (p.user._id || p.user.id || p.user));
                    return validMembers.length > 1;
                }
                const other = (c.participants || []).find(
                    (p: any) => p.user && String(p.user?._id || p.user?.id || p.user) !== currentUserId
                )?.user;
                return !!other;
            });
            ...
            set((state) => {
                ...
                const merged = fetched.map((incoming: any) => {
        ...
        set((state) => ({
            messages: state.messages.map((msg) => {
                ...
                const hasAttachment = (msg.attachments || []).some(
                    (att: any) => att.id === itemId || att._id === itemId || att.uploadId === itemId
                );
    ...
    receiveMessage: (message) => {
        ...
        if (isFromOther && !isActive) {
            ...
            if (!preview && message.attachments?.length) {
                const type = message.attachments[0].fileType || message.attachments[0].type || "";
    ...
    handleUserStatusEvent: ({ userId, status }) => {
        ...
        set((state) => {
            ...
            if (updatedActive?.participants) {
                updatedActive = {
                    ...updatedActive,
                    participants: updatedActive.participants.map((p: any) => {
                        const pId = (p.user?._id || p.user?.id || p.user)?.toString();
                        if (pId === targetId) {
                            return typeof p.user === "object" && p.user !== null
                                ? { ...p, user: { ...p.user, status } }
