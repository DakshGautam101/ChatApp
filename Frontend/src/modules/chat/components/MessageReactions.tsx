...
import type { MessageReactionInterface } from "@/core/types/MessageInterface";
...
interface MessageReactionsProps {
    reactions?: (MessageReactionInterface | any)[];
}

export function ReactionPicker({ isMine, onSelectReaction }: ReactionPickerProps) {
    return (
        <div
            className={cn(
...
export function MessageReactions({ reactions }: MessageReactionsProps) {
    ...
    const grouped = reactions.reduce<Record<string, (UserInterface | string | any)[]>>((acc, r: MessageReactionInterface) => {
        const type = r.type || r.emoji || r.reaction || "👍";
        if (!acc[type]) acc[type] = [];
        if (r.user) acc[type].push(r.user);
        return acc;
    }, {});
    ...
    return (
        <div className="flex flex-wrap gap-1 mt-0.5">
            {Object.entries(grouped).map(([emoji, users]) => {
                const isReactedByMe = users.some((u: UserInterface | string | any) => {
