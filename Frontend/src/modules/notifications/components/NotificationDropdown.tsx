    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            >
                ...
                {totalBadgeCount > 0 && (
                    <span className="absolute top-1 right-1 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-blue-600 px-1 text-[10px] font-extrabold text-white shadow-xs animate-pulse">
            ...
            {/* Dropdown Popover */}
            {isOpen && (
                <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-blue-900/10 z-50 overflow-hidden animate-scale-in">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50/50">
                        <div className="flex items-center gap-2">
                            <Bell className="h-4 w-4 text-blue-600" />
                            <h3 className="text-sm font-bold text-slate-900">Notifications</h3>
                            {unreadCount > 0 && (
                                <span className="badge badge-sm bg-blue-600 text-white font-extrabold border-none text-[10px]">
                                    {unreadCount} new
                                </span>
                            )}
                        </div>
                            ...
                            notifications.map((notif) => {
                                return (
                                    <div
                                        ...
                                        className={`flex items-start gap-3 p-3.5 transition-colors cursor-pointer hover:bg-slate-50 ${!notif.isRead ? "bg-blue-50/40" : ""
                                            }`}
                                    >
                                        <Avatar
                                        ...
                                        <div className="flex items-center gap-1 shrink-0 self-center">
                                            ...
                                            <button
                                                ...
                                                className="p-1 rounded-md text-slate-400 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
                                                title="Delete"
                                            >
                                                <Trash2 className="h-3.5 w-3.5" />
                                            </button>
